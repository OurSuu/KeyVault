document.addEventListener("DOMContentLoaded", () => {
  // ===== BASE_URL: Set API URL for Railway or Local =====
  const BASE_URL = window.location.origin.includes("railway.app")
    ? "https://keyvault-production-9151.up.railway.app"
    : "";
  ...
  if (signInBtn) {
    signInBtn.addEventListener("click", () => {
      ...
      popup.querySelector("#createAccountBtn").onclick = async () => {
        ...
        const send = await fetch(`${BASE_URL}/api/auth/send-otp`, {
          method: "POST",
        ...
        document.getElementById("verifyOtpBtn").onclick = async () => {
          ...
          const verify = await fetch(`${BASE_URL}/api/auth/verify-otp`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          const regRes = await registerUser({ name, email, password: pass });
