...
console.log("--- DEBUG: Attempting to require @getbrevo/brevo ---"); // <--- เพิ่ม
const SibApiV3Sdk = require('@getbrevo/brevo');
console.log("--- DEBUG: SibApiV3Sdk object:", SibApiV3Sdk); // <--- เพิ่ม
console.log("--- DEBUG: Type of SibApiV3Sdk:", typeof SibApiV3Sdk); // <--- เพิ่ม

// ตรวจสอบว่า ApiClient มีอยู่จริงไหม (ใส่ console.error)
if (SibApiV3Sdk && SibApiV3Sdk.ApiClient) {
    console.log("--- DEBUG: ApiClient check PASSED ---"); // <--- เพิ่ม (ถ้าปกติจะเห็นอันนี้)
} else {
    console.error("<<<<< FATAL: ApiClient is MISSING or UNDEFINED! >>>>>"); // <--- เพิ่ม (ถ้าผิดปกติจะเห็นอันนี้)
}

const defaultClient = SibApiV3Sdk.ApiClient.instance; // <--- Error เกิดตรงนี้
