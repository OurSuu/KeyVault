...
// ====== SendGrid Mail Client Setup ======
const sgMail = require('@sendgrid/mail');

// ตรวจสอบว่ามี API Key ใน Environment หรือไม่
if (!process.env.SENDGRID_API_KEY) {
    console.error("<<<<< FATAL: ไม่พบ SENDGRID_API_KEY ใน Environment Variables! >>>>>");
    // อาจจะ throw error หรือจัดการตามความเหมาะสม
    // throw new Error("จำเป็นต้องมี SENDGRID_API_KEY");
} else {
    sgMail.setApiKey(process.env.SENDGRID_API_KEY);
    console.log("--- DEBUG: ตั้งค่า SendGrid API Key เรียบร้อย ---");
}
// =====================================
...
app.post('/api/auth/register', async (req, res) => {
  const { email, password, name, otp } = req.body; // <-- รับ otp มาเหมือนเดิม

  // ตรวจสอบ input (ยังเช็คว่ามี otp ส่งมาด้วย เพื่อให้รู้ว่ามาจาก Flow ที่ถูกต้อง)
  if (!email || !password || !otp) {
    return res.status(400).json({ error: 'Email, password, and OTP are required (from verify step)' });
  }

  // ไม่ต้อง verify OTP ซ้ำตรงนี้แล้ว เพราะ /api/auth/verify-otp ทำไปแล้ว

  try {
    // ตรวจสอบแค่ว่า Email ซ้ำไหม แล้วสร้าง User ได้เลย
...
app.post("/api/auth/forgot-password", async (req, res) => {
  ...
  try {
    ...
    try {
      // 1. กำหนดข้อความสำหรับ SendGrid
      const msg = {
        to: email,
        from: {
           email: MAIN_EMAIL, // ต้องยืนยันใน SendGrid แล้ว
           name: 'KeyVault Support'
        },
      ...
      console.log("--- DEBUG: ส่งอีเมลรีเซ็ตรหัสผ่าน SendGrid สำเร็จไปที่:", email);

      res.json({ success: true });
    } catch (err) {
      console.error("<<<<< FATAL: SendGrid ส่งอีเมลรีเซ็ตรหัสผ่านล้มเหลว >>>>>");
      console.error("SendGrid Error:", err);
      if (err.response && err.response.body && err.response.body.errors) {
          console.error("SendGrid Error Body:", err.response.body.errors);
      }
      res.status(500).json({ error: "ส่งอีเมลรีเซ็ตไม่สำเร็จ (SendGrid Error)" });
    }
  } catch (err) {
...
app.post("/api/auth/send-otp", async (req, res) => {
  ...
  // +++ เพิ่มการ Normalize อีเมลตรงนี้ +++
  const normalizedEmail = email.trim().toLowerCase();
  console.log(`--- DEBUG SendOTP: Received Email: "${email}", Normalized to: "${normalizedEmail}" ---`);

  // ตรวจสอบ cooldown (ใช้อีเมลที่ Normalize แล้ว)
  const cooldown = otpManager.checkCooldown(normalizedEmail);
  if (cooldown.cooldown) {
  ...
  // ตั้งค่า cooldown (ใช้อีเมลที่ Normalize แล้ว)
  otpManager.setCooldown(normalizedEmail);

  // สร้าง OTP (ใช้อีเมลที่ Normalize แล้ว)
  const otp = otpManager.generateOtp(normalizedEmail);

  try {
    // 1. กำหนดรูปแบบข้อความสำหรับ SendGrid (ใช้ email เดิมที่ User กรอกมาก็ได้ หรือจะใช้ normalizedEmail ก็ได้)
    const msg = {
      to: email,
      from: {
        email: MAIN_EMAIL, // อีเมลนี้ต้องยืนยันใน SendGrid แล้ว
        name: 'KeyVault'
      },
    ...
    await sgMail.send(msg);
    // ใช้ normalizedEmail ใน Log เพื่อความสอดคล้อง
    console.log("--- DEBUG SendOTP: ส่งอีเมล SendGrid สำเร็จไปที่:", normalizedEmail);
    ...
    // ใช้ normalizedEmail ในการลบ cooldown
    otpManager.otpCooldown.delete(normalizedEmail);

    console.error("<<<<< FATAL: SendGrid ส่งอีเมลล้มเหลว >>>>>");
    console.error("SendGrid Error:", err);
    // แสดงรายละเอียด Error เพิ่มเติม ถ้ามี
    if (err.response && err.response.body && err.response.body.errors) {
        console.error("SendGrid Error Body:", err.response.body.errors);
    }
    res.status(500).json({ error: "ส่งอีเมลไม่สำเร็จ (SendGrid Error)" });
  }
});
...
app.post("/api/auth/verify-otp", (req, res) => {
  ...
  // --- DEBUGGING STARTS ---
  console.log(`--- DEBUG Verify: Received Email: "${email}", OTP: "${otp}" ---`);
  const normalizedEmail = email ? email.trim().toLowerCase() : null; // <--- ทำให้เป็นตัวเล็กและตัดเว้นวรรค
  console.log(`--- DEBUG Verify: Normalized Email: "${normalizedEmail}" ---`);

  // ดูว่ามีอะไรอยู่ใน Store บ้าง ณ ตอนนี้
  console.log("--- DEBUG Verify: Current otpStore keys:", Array.from(otpManager.otpStore.keys()));
  const storedRecord = otpManager.otpStore.get(normalizedEmail); // <--- ใช้ Email ที่ปรับแล้วในการค้นหา
  console.log("--- DEBUG Verify: Stored Record for this email:", storedRecord);
  // --- DEBUGGING ENDS ---

  // ใช้ normalizedEmail ในการตรวจสอบ
  const result = otpManager.verifyOtp(normalizedEmail, otp);

  if (!result.valid) {
    // --- DEBUGGING: Log the specific failure reason ---
    console.error("--- DEBUG Verify: Verification FAILED. Reason:", result.error);
    // --- DEBUGGING ENDS ---
    return res.status(400).json({ error: result.error || "รหัส OTP ไม่ถูกต้องหรือหมดอายุ" }); // ส่ง error ที่ได้จาก verifyOtp กลับไป
  }

  // --- DEBUGGING: Log success ---
  console.log("--- DEBUG Verify: Verification SUCCESSFUL ---");
  // --- DEBUGGING ENDS ---
