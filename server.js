...
// ====== Brevo API Client Setup (Using Named Exports) ======
console.log("--- DEBUG: Attempting to require @getbrevo/brevo ---");
const brevoPackage = require('@getbrevo/brevo');
console.log("--- DEBUG: require('@getbrevo/brevo') successful ---");

// 1. ลองดึง ApiClient และ TransactionalEmailsApi ออกมาโดยตรง
const ApiClient = brevoPackage.ApiClient;
const TransactionalEmailsApi = brevoPackage.TransactionalEmailsApi;

// 2. ตรวจสอบว่าดึงออกมาได้หรือไม่
if (typeof ApiClient !== 'function' || typeof ApiClient.instance !== 'object') {
    console.error("<<<<< FATAL: Failed to extract ApiClient or ApiClient.instance! >>>>>");
    console.error("--- DEBUG: Extracted ApiClient:", ApiClient);
    console.error("--- DEBUG: Full brevoPackage:", brevoPackage); // Log ดูว่ามีอะไรบ้าง
    throw new Error("Brevo SDK structure mismatch: Could not access ApiClient correctly.");
}
console.log("--- DEBUG: ApiClient extraction successful ---");

if (typeof TransactionalEmailsApi !== 'function') {
    console.error("<<<<< FATAL: Failed to extract TransactionalEmailsApi! >>>>>");
    console.error("--- DEBUG: Extracted TransactionalEmailsApi:", TransactionalEmailsApi);
    throw new Error("Brevo SDK structure mismatch: Could not access TransactionalEmailsApi correctly.");
}
console.log("--- DEBUG: TransactionalEmailsApi extraction successful ---");

// 3. ใช้ ApiClient ที่ดึงออกมาเพื่อตั้งค่า Key
const apiClientInstance = ApiClient.instance;
const apiKeyAuth = apiClientInstance.authentications['api-key'];

// 4. ตรวจสอบก่อนตั้งค่า Key
if (!apiKeyAuth) {
     console.error("<<<<< FATAL: apiClientInstance.authentications['api-key'] is MISSING! >>>>>");
     console.error("--- DEBUG: apiClientInstance.authentications:", apiClientInstance.authentications);
...
// 5. สร้าง Instance โดยใช้ TransactionalEmailsApi ที่ดึงออกมา
...
app.post("/api/auth/forgot-password", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const sendSmtpEmail = new brevoPackage.SendSmtpEmail();
